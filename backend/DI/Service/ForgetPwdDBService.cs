...
namespace DI.Service
{
    public class ForgetPwdDBService
    {
        ...
        public int forget_pwd(string user_email)
        {
            //查詢一筆資料(判斷資料庫是否有此email)
            SqlConnection conn = new SqlConnection(connectionString);
            //Sql 語法
            string sql = $@"SELECT COUNT(*) FROM ""user"" where user_email ='{user_email}' and user_start='1' ";
            conn.Open();
            ...
            if (num > 0)
            {
                ...
                //取使用者姓名
                string name_sql = $@"SELECT user_name FROM ""user"" where user_email ='{user_email}' ";
                ...
                //修改資料庫驗證碼欄位
                string upd_pwd_sql = $@"UPDATE ""user"" SET user_authcode='{ValidateCode}' WHERE user_email ='{user_email}'";
        ...
        #region 驗證驗證碼
        public string forget_pwd_code(string user_email, string code)
        {
            SqlConnection conn = new SqlConnection(connectionString);
            string sql = $@"SELECT * FROM ""user"" where user_email ='{user_email}' and user_start='1' ";
            SqlCommand command = new SqlCommand(sql, conn);
            conn.Open();
