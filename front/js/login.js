function nexttt(){
    ...
    // if((user.value === "123") & (pwd.value === "123")){
    //     alert("登入成功!");
    ...
    axios.post('https://localhost:7094/api/User/Login', formData, {
        headers: {
      ...
      .then(response => {
        ...
        if(response.status === 200 & response.data.user.user_level == true){
            alert("即將進入會員畫面");
        ...
        }else if(response.status === 200 & response.data.user.user_level == false){
            alert("即將進入管理員畫面");
            ...
            console.log(admin);
            window.location.href="../back/back-index.html";
        }
      })
      .catch(error => {
